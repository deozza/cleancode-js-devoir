...
describe('resolveCombatRound', () => {
	...
	it('should throw an error when max reroll is reached', () => {
		...
		expect(() => rerollWeapon(currentWeapon, weaponList, usedWeapons, rerollCount)).toThrow(
	...
	it('should throw an error if no weapons are available for reroll', () => {
		...
		expect(() => rerollWeapon(currentWeapon, weaponList, usedWeapons, rerollCount)).toThrow(
			"Plus d'armes disponibles pour la relance"
		);
	});
});
